...
public class ReporteProveedores extends ActionBarActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Cursor cPro1 = base.rawQuery("SELECT * FROM tbl_proveedor WHERE id_emp='"+idEmp+"'", null);

		if (cPro1.moveToFirst()) {
			do {
				prov = new Proveedor(cPro1.getInt(0),cPro1.getString(2), cPro1.getString(3));
				listaPro.add(prov);
			} while (cPro1.moveToNext());
		}

		startManagingCursor(cPro1);

		//===========================================================================================
		
		
		for (int j = listaPro.size()-1; j >= 0; j--) {
		...
		Cursor c = base.rawQuery("SELECT * FROM tbl_compra WHERE id_emp='"+idEmp+"' AND id_pro='"+listaPro.get(j).id+"'", null);
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		getMenuInflater().inflate(R.menu.reporte_mensual, menu);
		return true;
	}
